...
const EditarPedidosXVentasForm = ({navigation}) => {
	...
	const {idRegistro} = useContext(PedidosXVentasContext)
	...
	const {idPedidosXVentas} = useContext(PedidosXVentasContext)
	...
	const editarPedidosXVentas = async () => {
		if (!token) {
			...
			console.log( NumeroFactura, NumeroPedido, idPedidosXVentas );
			await Axios.put(
				"pedidosyVentas/editar?id=" + idPedidosXVentas,
				bodyParameters,
				config
			)
				.then((data) => {
...
export default EditarPedidosXVentasForm
